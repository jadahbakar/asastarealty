...
const Register = props => {
  ...
  const showAlert = (kind, title, message) => {
    ...
    if (kind === 'ok') {
      setAlert(
        <SweetAlert success title={title} onConfirm={() => hideAlert()}>
          {message}
        </SweetAlert>
      )
    }
    ...
    if (kind === 'fail') {
      setAlert(
        <SweetAlert danger title={title} onConfirm={() => hideAlert()}>
          {message}
        </SweetAlert>
      )
    }
  ...
  const onSubmit = data => {
    ...
    axios
      ...
      .then(response => {
        showAlert('ok', 'Success', `UserId Anda : ${response.data}`)
      })
      .catch(error => {
        showAlert('fail', error.response.status, error.response.data.message)
      })
  }
